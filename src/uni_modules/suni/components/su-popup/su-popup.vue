<template>
  ...
  <view v-if="!lazyRender || inited" :class="rootClass" :style="style" @transitionend="onTransitionEnd">
    <slot />
    <su-icon v-if="closable" custom-class="su-popup__close" name="add" @click="close" />
  </view>
</template>
...
const getClassNames = (name?: string) => {
  ...
  return {
    enter: `su-${name}-enter su-${name}-enter-active`,
    'enter-to': `su-${name}-enter-to su-${name}-enter-active`,
    leave: `su-${name}-leave su-${name}-leave-active`,
    'leave-to': `su-${name}-leave-to su-${name}-leave-active`
  }
}
...
const rootClass = computed(() => {
  return `su-popup su-popup--${props.position} ${props.customClass || ''} ${classes.value || ''}`
})
