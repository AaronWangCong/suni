    ...
    >
      <view :class="rootClass">
        <view :class="bodyClass">
          <view v-if="title" class="su-message-box__title">
            {{ title }}
          </view>
          <view class="su-message-box__content">
            <block v-if="type === 'prompt'">
              <su-input v-model="inputValue" :type="inputType" size="large" :placeholder="inputPlaceholder || '请输入'" @input="inputValChange" />
              <view v-if="showErr" class="su-message-box__input-error">
        ...
        <view :class="`su-message-box__actions ${showCancelButton ? 'su-message-box__flex' : 'su-message-box__block'}`">
          <su-button type="info" block v-if="showCancelButton" custom-style="margin-right: 16px;" @click="toggleModal('cancel')">
            {{ cancelButtonText || translate('cancel') }}
          </su-button>
          <su-button block @click="toggleModal('confirm')">
...
import wdPopup from '../su-popup/su-popup.vue'
import wdButton from '../su-button/su-button.vue'
import wdInput from '../su-input/su-input.vue'
...
const rootClass = computed(() => {
  return `su-message-box__container ${props.customClass}`
})

const bodyClass = computed(() => {
  return `su-message-box__body ${!title.value ? 'is-no-title' : ''} ${type.value === 'prompt' ? 'is-prompt' : ''}`
})
