<template>
  <view :class="`su-col-picker ${cell.border.value ? 'is-border' : ''} ${customClass}`" :style="customStyle">
    <view class="su-col-picker__field" @click="showPicker">
      <slot v-if="useDefaultSlot"></slot>
      <view
        v-else
        :class="`su-col-picker__cell ${disabled && 'is-disabled'} ${readonly && 'is-readonly'} ${alignRight && 'is-align-right'} ${
          error && 'is-error'
        }  ${size && 'is-' + size}`"
      >
        <view
          v-if="label || useLabelSlot"
          :class="`su-col-picker__label ${isRequired && 'is-required'} ${customLabelClass}`"
          :style="labelWidth ? 'min-width:' + labelWidth + ';max-width:' + labelWidth + ';' : ''"
        >
          <block v-if="label">{{ label }}</block>
          <slot v-else name="label"></slot>
        </view>
        <view class="su-col-picker__body">
          <view class="su-col-picker__value-wraper">
            <view
              :class="`su-col-picker__value ${ellipsis && 'is-ellipsis'} ${customValueClass} ${showValue ? '' : 'su-col-picker__value--placeholder'}`"
            >
              {{ showValue || placeholder || translate('placeholder') }}
            </view>
            <su-icon v-if="!disabled && !readonly" custom-class="su-col-picker__arrow" name="arrow-right" />
          </view>
          <view v-if="errorMessage" class="su-col-picker__error-message">{{ errorMessage }}</view>
        </view>
      </view>
    </view>
    <su-action-sheet
    ...
    >
      <view class="su-col-picker__selected">
        <scroll-view :scroll-x="true" scroll-with-animation :scroll-left="scrollLeft">
          <view class="su-col-picker__selected-container">
            <view
              v-for="(_, colIndex) in selectList"
              :key="colIndex"
              :class="`su-col-picker__selected-item  ${colIndex === currentCol && 'is-selected'}`"
              @click="handleColClick(colIndex)"
            >
              {{ selectShowList[colIndex] || translate('select') }}
            </view>
            <view class="su-col-picker__selected-line" :style="state.lineStyle"></view>
          </view>
        </scroll-view>
      </view>
      <view class="su-col-picker__list-container">
        <view
          v-for="(col, colIndex) in selectList"
          :key="colIndex"
          class="su-col-picker__list"
          :style="colIndex === currentCol ? 'display: block;' : 'display: none;'"
        >
          <view
            v-for="(item, index) in col"
            :key="index"
            :class="`su-col-picker__list-item ${pickerColSelected[colIndex] && item[valueKey] === pickerColSelected[colIndex] && 'is-selected'} ${
              item.disabled && 'is-disabled'
            }`"
            @click="chooseItem(colIndex, index)"
          >
            <view>
              <view class="su-col-picker__list-item-label">{{ item[labelKey] }}</view>
              <view v-if="item[tipKey]" class="su-col-picker__list-item-tip">{{ item[tipKey] }}</view>
            </view>
            <su-icon custom-class="su-col-picker__checked" name="check"></su-icon>
          </view>
          <view v-if="loading" class="su-col-picker__loading">
            <su-loading :color="loadingColor" />
          </view>
        </view>
      </view>
    </su-action-sheet>
...
import suLoading from '../su-loading/su-loading.vue'
import suActionSheet from '../su-action-sheet/su-action-sheet.vue'
...
import { FORM_KEY, type FormItemRule } from '../su-form/types'
...
const $container = '.su-col-picker__selected-container'
const $item = '.su-col-picker__selected-item'
...
watch(
  ...
  (newValue, oldValue) => {
    if (newValue.length && !isArray(newValue[0])) {
      console.error('[wot design] error(su-col-picker): the columns props of su-col-picker should be a two-dimensional array')
      return
    }
    ...
    columnChange({
      ...
      resolve: (nextColumn: Record<string, any>[]) => {
        if (!isArray(nextColumn)) {
          console.error('[wot design] error(su-col-picker): the data of each column of su-col-picker should be an array')
          return
        }
