...
declare interface InputSlots {
  /**
   * 输入框前置内容，`nuve`环境需`su-input`有效，非`nvue`环境需`su-input`才有效
   */
  ['prefix']?: () => any
  /**
   * 输入框后置内容，`nuve`环境需`su-input`有效，非`nvue`环境需`su-input`才有效
   */
  ['suffix']?: () => any
}
...
declare interface _Input {
  new (): {
    $props: AllowedComponentProps & VNodeProps & InputProps
    $slots: InputSlots
  }
}
