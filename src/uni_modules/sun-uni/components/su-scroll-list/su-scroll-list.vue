<template>
	<view
		class="su-scroll-list"
		ref="su-scroll-list"
	>
		<!-- #ifdef APP-NVUE -->
		<!-- nvue使用bindingX实现，以得到更好的性能 -->
		<scroller
			class="su-scroll-list__scroll-view"
			ref="su-scroll-list__scroll-view"
			scroll-direction="horizontal"
			:show-scrollbar="false"
			:offset-accuracy="1"
			@scroll="nvueScrollHandler"
		>
			<view class="su-scroll-list__scroll-view__content">
				<slot />
			</view>
		</scroller>
		<!-- #endif -->
		...
		<!-- 以上平台，支持wxs -->
		<scroll-view
			class="su-scroll-list__scroll-view scroll-view-native"
			scroll-x
			enable-flex
			@scroll="wxs.scroll"
		...
		>
			...
			<!-- 非以上平台，只能使用普通js实现 -->
			<scroll-view
				class="su-scroll-list__scroll-view scroll-view-js"
				scroll-x
			...
			<!-- #endif -->
			<view
				class="su-scroll-list__indicator"
				v-if="indicator"
				:style="[addStyle(indicatorStyle)]"
			>
				<view
					class="su-scroll-list__indicator__line"
					:style="[lineStyle]"
				>
					<view
						class="su-scroll-list__indicator__line__bar"
						:style="[barStyle]"
						ref="su-scroll-list__indicator__line__bar"
					></view>
				</view>
			</view>
	</view>
 ...
 * @tutorial https://suni.pages.dev/sun-uni/component/scrollList.html
...
export default {
	...
	methods: {
		...
		async getComponentWidth() {
			...
			await sleep(30)
			// #ifndef APP-NVUE
			this.$uGetRect('.su-scroll-list').then(size => {
				this.scrollWidth = size.width
			})
			// #endif

			// #ifdef APP-NVUE
			const ref = this.$refs['su-scroll-list']
