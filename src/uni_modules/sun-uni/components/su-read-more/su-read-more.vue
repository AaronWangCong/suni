<template>
  <view class="su-read-more">
    <view
      class="su-read-more__content"
      :style="{
        height: isLongContent && status === 'close' ? addUnit(showHeight) : addUnit(contentHeight),
        textIndent: textIndent
      }"
    >
      <view class="su-read-more__content__inner" ref="su-read-more__content__inner" :class="[elId]">
        <slot></slot>
      </view>
    </view>
    <view class="su-read-more__toggle" :style="[innerShadowStyle]" v-if="isLongContent">
      <slot name="toggle">
        <view class="su-read-more__toggle__text" @tap="toggleReadMore">
          <su-text
            ...
            margin="0 5px 0 0"
          ></su-text>
          <view class="su-read-more__toggle__icon">
            <su-icon :color="color" :size="fontSize + 2" :name="status === 'close' ? 'arrow-down' : 'arrow-up'"></su-icon>
          </view>
        </view>
      </slot>
    </view>
 ...
 * @tutorial https://suni.pages.dev/sun-uni/component/readMore.html
 ...
 * @example <su-read-more><rich-text :nodes="content"></rich-text></su-read-more>
 */
export default {
  name: 'su-read-more',
  ...
  methods: {
    ...
    async init() {
      this.getContentHeight().then((height) => {
        ...
        if (height > getPx(this.showHeight)) {
          ...
          // https://github.com/ijry/sun-uni/issues/270
    ...
    async getContentHeight() {
      ...
      return new Promise((resolve) => {
        ...
        // #ifdef APP-NVUE
        const ref = this.$refs['su-read-more__content__inner']
