...
Parser.prototype.onOpenTag = function(selfClose) {
	...
	if (node.name === 'embed') {
		...
		if (src.includes('.mp4') || src.includes('.3gp') || src.includes('.m3u8') || (attrs.type || '').includes(
				'video')) {
			node.name = 'video'
		} else if (src.includes('.mp3') || src.includes('.wav') || src.includes('.aac') || src.includes('.m4a') || (attrs
				.type || '').includes('audio')) {
			node.name = 'audio'
		}
		if (attrs.autostart) {
	...
	if (close) {
		if (!this.hook(node) || config.ignoreTags[node.name]) {
			...
			else if (node.name === 'source' && parent && (parent.name === 'video' || parent.name === 'audio') && attrs
				.src) {
		...
		if (node.name === 'img') {
			...
			if (!isNaN(parseInt(styleObj.height)) && (!styleObj.height.includes('%') || (parent && (parent.attrs.style ||
					'').includes('height')))) {
				node.h = 'T'
			}
		...
		if ((node.name === 'pre' || ((attrs.style || '').includes('white-space') && attrs.style.includes('pre'))) && this
			.pre !== 2) {
			this.pre = node.pre = 1
		}
		node.children = []
	...
	if (
			...
			for (let row = 1; row <= trList.length; row++) {
				...
				for (let j = 0; j < trList[row - 1].children.length; j++) {
					...
					if (td.name === 'td' || td.name === 'th') {
						...
						style =
							(border ?
								`;border:${border}px ${borderstyle || 'solid'} ${bordercolor || 'gray'}` + (spacing ? '' :
		...
		(function traversal(node) {
			...
			for (let i = node.children.length; i--;) {
				...
				// #ifdef (MP-WEIXIN || MP-QQ || APP-PLUS || MP-360) && VUE3
				if (child.name && (config.inlineTags[child.name] || ((child.attrs.style || '').includes('inline') && child
						.children)) && !child.c) {
					traversal(child)
				}
				// #endif
