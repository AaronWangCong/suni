<template>
  <view class="su-subsection" ref="su-subsection" :class="[`su-subsection--${mode}`]" :style="[addStyle(customStyle), wrapperStyle]">
    <view
      class="su-subsection__bar cursor-pointer"
      ref="su-subsection__bar"
      :style="[barStyle]"
      :class="[
        mode === 'button' && 'su-subsection--button__bar',
        innerCurrent === 0 && mode === 'subsection' && 'su-subsection__bar--first',
        innerCurrent > 0 && innerCurrent < list.length - 1 && mode === 'subsection' && 'su-subsection__bar--center',
        innerCurrent === list.length - 1 && mode === 'subsection' && 'su-subsection__bar--last'
      ]"
    ></view>
    <view
      class="su-subsection__item cursor-pointer"
      :class="[
        `su-subsection__item--${index}`,
        index < list.length - 1 && 'su-subsection__item--no-border-right',
        index === 0 && 'su-subsection__item--first',
        index === list.length - 1 && 'su-subsection__item--last'
      ]"
      :ref="`su-subsection__item--${index}`"
      :style="[itemStyle(index)]"
      @tap="clickHandler(index)"
      v-for="(item, index) in list"
      :key="index"
    >
      <text class="su-subsection__item__text" :style="[textStyle(index)]">{{ getText(item) }}</text>
    </view>
  </view>
</template>
 ...
 * @tutorial https://suni.pages.dev/sun-uni/component/subsection.html
 ...
 * @example <su-subsection :list="list" :current="curNow" @change="sectionChange"></su-subsection>
 */
export default {
  name: 'su-subsection',
  ...
  computed: {
    ...
    textStyle(index) {
      return (index) => {
        ...
        if (this.mode === 'subsection') {
          style.color = this.innerCurrent === index ? '#fff' : this.inactiveColor
        } else {
          // button模式下，激活时文字颜色默认为activeColor
          style.color = this.innerCurrent === index ? this.activeColor : this.inactiveColor
        }
        return style
      }
    }
  ...
  methods: {
    ...
    getRect() {
      // #ifndef APP-NVUE
      this.$uGetRect('.su-subsection__item--0').then((size) => {
        this.itemRect = size
      })
      // #endif

      // #ifdef APP-NVUE
      const ref = this.$refs['su-subsection__item--0'][0]
      ref &&
