<template>
  <view
    ...
    :class="'su-lazy-item-' + elIndex"
  >
    <view :class="'su-lazy-item-' + elIndex">
      <image
...
<script>
import { addUnit, guid } from '../../libs/function/index'
/**
 * lazyLoad 懒加载
 * @description 懒加载使用的场景为：页面有很多图片时，APP会同时加载所有的图片，导致页面卡顿，各个位置的图片出现前后不一致等.
 * @tutorial https://suni.pages.dev/component/lazy-load.html
 ...
 * @example <su-lazy-load :image="image" :loading-img="loadingImg" :error-img="errorImg"></su-lazy-load>
 */
export default {
  name: 'su-lazy-load',
  props: {
  ...
  mounted() {
    ...
    setTimeout(() => {
      ...
      contentObserver
        ...
        .observe('.su-lazy-item-' + this.elIndex, (res) => {
