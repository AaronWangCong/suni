<template>
  <view class="su-textarea" :class="textareaClass" :style="[textareaStyle]">
    <textarea
      class="su-textarea__field"
      :value="innerValue"
 ...
 * @tutorial https://suni.pages.dev/sun-uni/component/textarea.html
 *
 ...
 * @example <su-textarea v-model="value1" placeholder="请输入内容" ></su-textarea>
 */
export default {
  name: 'su-textarea',
  ...
  computed: {
    ...
    textareaClass() {
      ...
      border === 'surround' && (classes = classes.concat(['su-border', 'su-textarea--radius']))
      border === 'bottom' && (classes = classes.concat(['su-border-bottom', 'su-textarea--no-radius']))
      disabled && classes.push('su-textarea--disabled')
      return classes.join(' ')
    },
    // 组件的样式
    textareaStyle() {
