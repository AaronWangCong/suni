<template>
  <view class="su-skeleton">
    <view class="su-skeleton__wrapper" ref="su-skeleton__wrapper" v-if="loading" style="display: flex; flex-direction: row">
      <view
        class="su-skeleton__wrapper__avatar"
        v-if="avatar"
        :class="[`su-skeleton__wrapper__avatar--${avatarShape}`, animate && 'animate']"
        :style="{
          height: addUnit(avatarSize),
          width: addUnit(avatarSize)
        }"
      ></view>
      <view class="su-skeleton__wrapper__content" ref="su-skeleton__wrapper__content" style="flex: 1">
        <view
          class="su-skeleton__wrapper__content__title"
          v-if="title"
          :style="{
            width: uTitleWidth,
        ...
        ></view>
        <view
          class="su-skeleton__wrapper__content__rows"
          :class="[animate && 'animate']"
 ...
 * @tutorial https://suni.pages.dev/sun-uni/component/skeleton.html
...
export default {
  ...
  methods: {
    ...
    async setNvueAnimation() {
      ...
      await sleep(500)
      const skeleton = this.$refs['su-skeleton__wrapper']
      skeleton &&
        this.loading &&
        this.animate &&
    ...
    async getComponentWidth() {
      ...
      await sleep(20)
      // #ifndef APP-NVUE
      this.$uGetRect('.su-skeleton__wrapper__content').then((size) => {
        this.width = size.width
      })
      // #endif

      // #ifdef APP-NVUE
      const ref = this.$refs['su-skeleton__wrapper__content']
      ref &&
