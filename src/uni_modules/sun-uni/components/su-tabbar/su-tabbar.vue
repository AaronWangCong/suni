<template>
  <view class="su-tabbar">
    <view
      class="su-tabbar__content"
      ref="su-tabbar__content"
      @touchmove.stop.prevent="noop"
      :class="[border && 'su-border-top', fixed && 'su-tabbar--fixed']"
      :style="[tabbarStyle]"
    >
      <view class="su-tabbar__content__item-wrapper">
        <slot />
      </view>
      <su-safe-bottom v-if="safeAreaInsetBottom"></su-safe-bottom>
    </view>
    <view
      class="su-tabbar__placeholder"
      v-if="placeholder"
      :style="{
 ...
 * @tutorial https://suni.pages.dev/sun-uni/component/tabbar.html
 ...
 *
 * @example <su-tabbar :value="value2" :placeholder="false" @change="name => value2 = name" :fixed="false" :safeAreaInsetBottom="false"><su-tabbar-item text="首页" icon="home" dot ></su-tabbar-item></su-tabbar>
 */
export default {
  name: 'su-tabbar',
  ...
  methods: {
    ...
    async setPlaceholderHeight() {
      ...
      await sleep(20)
      // #ifndef APP-NVUE
      this.$uGetRect('.su-tabbar__content').then(({ height = 50 }) => {
      ...
      // #ifdef APP-NVUE
      dom.getComponentRect(this.$refs['su-tabbar__content'], (res) => {
        const { size } = res
