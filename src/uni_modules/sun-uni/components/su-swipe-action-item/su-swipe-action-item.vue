<template>
  ...
  <!-- #endif -->
  <view class="su-swipe-action-item" ref="su-swipe-action-item">
    <view class="su-swipe-action-item__right">
      <slot name="button">
        <view
          v-for="(item, index) in options"
          :key="index"
          class="su-swipe-action-item__right__button"
          :ref="`su-swipe-action-item__right__button-${index}`"
          :style="[
            {
        ...
        >
          <view
            class="su-swipe-action-item__right__button__wrapper"
            :style="[
              {
          ...
          >
            ...
            <text
              v-if="item.text"
              class="su-swipe-action-item__right__button__wrapper__text su-line-1"
              :style="[
                {
                  color: item.style && item.style.color ? item.style.color : '#ffffff',
    ...
    <view
      class="su-swipe-action-item__content"
    ...
    <!-- <view class="su-swipe-action-item__content" @touchstart="mysjs.touchstart" @touchmove="mysjs.touchmove"
			@touchend="mysjs.touchend">
			<slot></slot>
		</view> -->
    <!-- #endif -->
    <!-- #ifdef APP-NVUE -->
    <view class="su-swipe-action-item__content" ref="su-swipe-action-item__content" @panstart="onTouchstart" @tap="clickHandler">
      <slot></slot>
    </view>
    <!-- #endif -->
  </view>
 ...
 * @tutorial https://suni.pages.dev/sun-uni/component/swipeAction.html
 ...
 * @example	<su-swipe-action><su-swipe-action-item :options="options1" ></su-swipe-action-item></su-swipe-action>
 */
export default {
  name: 'su-swipe-action-item',
  ...
  methods: {
    ...
    updateParentData() {
      // 此方法在mixin中
      this.getParentData('su-swipe-action')
    },
    // #ifndef APP-NVUE
    // 查询节点
    queryRect() {
      this.$uGetRect('.su-swipe-action-item__right__button', true).then((buttons) => {
        this.size = {
          buttons,
          show: this.show,
