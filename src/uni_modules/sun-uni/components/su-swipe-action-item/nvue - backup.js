  ...
  methods: {
    ...
    moveSwipeAction(moveX) {
      ...
      animation.transition(
        this.$refs['su-swipe-action-item__content'].ref,
        {
          styles: {
      ...
      for (let i = len - 1; i >= 0; i--) {
        const buttonRef = this.$refs[`su-swipe-action-item__right__button-${i}`][0].ref
        // 通过比例，得出元素自身该移动的距离
    ...
    queryRect() {
      ...
      const promiseAll = this.rightOptions.map((item, index) => this.getRectByDom(this.$refs[`su-swipe-action-item__right__button-${index}`][0]))
