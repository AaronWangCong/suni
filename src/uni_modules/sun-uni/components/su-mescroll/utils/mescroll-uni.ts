import type { MeScrollDownOptions, MeScrollOptions, MeScrollUpOptions } from '../../../types/comps/mescroll'

export default class MeScroll {
  /** 配置项 */
  options: MeScrollOptions = {}
  /** 版本号 */
  version: string = '1.0.0'
  /** 是否滚动的是body */
  isScrollBody: boolean = false
  /** 是否正在下拉刷新 */
  isDownScrolling: boolean = false
  /** 是否正在上拉加载 */
  isUpScrolling: boolean = false
  /** 下拉刷新的配置选项 */
  optDown: MeScrollDownOptions = {}
  /** 是否自动上拉加载 */
  isUpAutoLoad: boolean = false
  /** 拉刷新的高度 */
  downHight: number = 0
  /** 下拉刷新是否成功 */
  isDownEndSuccess: boolean | null = null // 下拉加载是否成功
  /** 上拉加载的配置选项 */
  optUp: MeScrollUpOptions = {}
  /** 视图ID */
  viewId: string = ''
  /** 国际化配置 */
  i18n: MeScrollOptions['i18n'] = {}

  constructor(options: MeScrollOptions = {}, isScrollBody?: boolean) {
    const me = this
    me.options = options || {}
    me.isScrollBody = isScrollBody || false
    const hasDownCallback = options.down && options.down.callback
    me.initDownScroll()
    me.initUpScroll()
  ...
  static extend(userOption: { [x: string]: any }, defaultOption: { [x: string]: any }) {
  ...
  /* 配置参数:下拉刷新 */
  extendDownScroll(optDown: MeScrollDownOptions) {
    MeScroll.extend(optDown, {
      ...
      callback: function (mescroll: MeScroll) {
  ...
  /* 配置参数:上拉加载 */
  extendUpScroll(optUp: MeScrollUpOptions) {
    // 上拉加载的配置
  ...
  /** 简单判断是否配置了颜色 (非透明,非白色) */
  hasColor = function (color?: string) {
  ...
  /** 列表touchstart事件 */
  touchstartEvent = function (e: HTMLElement) {
  ...
  /** 列表touchmove事件 */
  touchmoveEvent = function (e: HTMLElement) {
    if (!this.optDown.use) return
    let me = this
  ...
  /** 列表touchend事件 */
  touchendEvent = function (e: HTMLElement) {
    let me = this
    if (!me.optDown.use) return
    // 如果下拉区域高度已改变,则需重置回来
    if (me.isMoveDown) {
      if (me.downHight >= me.optDown.offset) {
        // 符合触发刷新的条件
        ...
        // 不符合的话 则重置
        me.downHight = 0
        me.endDownScrollCall(this)
      }
      me.movetype = 0
      me.isMoveDown = false
    } else if (!me.isScrollBody && me.getScrollTop() === this.startTop) {
      // scroll-view到顶/左/右/底的滑动事件
      let isScrollUp = me.getPoint(e).y - me.startPoint.y < 0 // 和起点比,移动的距离,大于0向下拉,小于0向上拉
      // 上滑
      if (isScrollUp) {
        // 需检查滑动的角度
        let angle = me.getAngle(me.getPoint(e), me.startPoint) // 两点之间的角度,区间 [0,90]
        if (angle > 80) {
          // 检查并触发上拉
  ...
  /** 计算两点之间的角度: 区间 [0,90] */
  getAngle = function (p1: { x: number; y: number }, p2: { x: number; y: number }) {
  ...
  /** 下拉加载中 */
  showDownLoadingCall = function (downHight: any) {
  ...
  /** 上拉加载 */
  endDownScrollCall = function () {
    this.optDown.endDownScroll && this.optDown.endDownScroll(this)
    this.optDown.afterEndDownScroll && this.optDown.afterEndDownScroll(this)
  }

  /** 锁定下拉刷新:isLock=ture,null锁定;isLock=false解锁  */
  lockDownScroll = function (isLock: boolean) {
    if (isLock == null) isLock = true
    this.optDown.isLock = isLock
  }

  /**  锁定上拉加载:isLock=ture,null锁定;isLock=false解锁  */
  lockUpScroll = function (isLock: boolean) {
    if (isLock == null) isLock = true
  ...
  onPageScroll = function (e: Page.PageScrollOption) {
  ...
  /** 列表滚动事件 */
  scroll = function (e: HTMLElement, onScroll: Function) {
    // 更新滚动条的位置
    this.setScrollTop(e.scrollTop)
    // 更新滚动内容高度
  ...
  /** 触发上拉加载 */
  triggerUpScroll = function (isCheck?: boolean) {
  ...
  /** 结束上拉加载 */
  endUpScroll = function (isShowNoMore: Boolean) {
   ...
   */
  resetUpScroll = function (isShowLoading?: boolean) {
  ...
  /** 设置page.num的值 */
  setPageNum = function (num: number) {
    this.optUp.page.num = num - 1
  }

  /** 设置page.size的值 */
  setPageSize = function (size: number) {
   ...
   * systime: 服务器时间 (可空)
   */
  endByPage = function (dataSize: number, totalPage: number, systime: number) {
    let hasNext
   ...
   * systime: 服务器时间 (可空)
   */
  endBySize = function (dataSize: number, totalSize: number, systime?: number) {
    let hasNext
   ...
   */
  endSuccess = function (dataSize?: number, hasNext?: boolean, systime?: number) {
    let me = this
    // 结束下拉刷新
  ...
  /** 回调失败,结束下拉刷新和上拉加载 */
  endErr = function (errDistance?: any) {
  ...
  /** 记录滚动条的位置 */
  setScrollTop = function (y: number) {
    this.scrollTop = y
  }

  /** 滚动到指定位置 */
  scrollTo = function (y: number, t: number) {
    this.myScrollTo && this.myScrollTo(y, t) // scrollview需自定义回到顶部方法
  }

  /** 自定义scrollTo */
  resetScrollTo = function (myScrollTo: any) {
 ...
 * */
  getStep = function (star: number, end: number, callback: Function, t: number, rate?: number) {
  ...
  /** 滚动容器的高度 */
  getClientHeight = function (isReal: boolean) {
  ...
  /** 设置高度 */
  setClientHeight = function (h: number) {
    this.clientHeight = h
  }
  ...
  /** 滚动的高度 */
  setScrollHeight = function (h: number) {
    this.scrollHeight = h
  }
  ...
  /** 设置body高度 */
  setBodyHeight = function (h: number) {
    this.bodyHeight = h
  }
