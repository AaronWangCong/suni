<template>
  ...
  >
    <view class="su-keyboard">
      <slot />
      <view class="su-keyboard__tooltip" v-if="tooltip">
        <view hover-class="su-hover-class" :hover-stay-time="100">
          <text class="su-keyboard__tooltip__item su-keyboard__tooltip__cancel" v-if="showCancel" @tap="onCancel">
            {{ showCancel && cancelText }}
          </text>
        </view>
        <view>
          <text v-if="showTips" class="su-keyboard__tooltip__item su-keyboard__tooltip__tips">
            {{ tips ? tips : mode == 'number' ? '数字键盘' : mode == 'card' ? '身份证键盘' : '车牌号键盘' }}
          </text>
        </view>
        <view hover-class="su-hover-class" :hover-stay-time="100">
          <text v-if="showConfirm" @tap="onConfirm" class="su-keyboard__tooltip__item su-keyboard__tooltip__submit" hover-class="su-hover-class">
            {{ showConfirm && confirmText }}
          </text>
        </view>
      </view>
      <template v-if="mode == 'number' || mode == 'card'">
        <su-number-keyboard :random="random" @backspace="backspace" @change="change" :mode="mode" :dotDisabled="dotDisabled"></su-number-keyboard>
      </template>
      <template v-else>
        <su-car-keyboard :modelValue="defaultValue" @change="change" @confirm="onConfirm" @cancel="onCancel"></su-car-keyboard>
      </template>
    </view>
  </su-popup>
</template>
 ...
 * @tutorial https://suni.pages.dev/sun-uni/component/keyboard.html
 ...
 * @example <su-keyboard mode="number" :show="show"></su-keyboard>
 */
