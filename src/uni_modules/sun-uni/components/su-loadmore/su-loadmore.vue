<template>
  ...
  >
    <su-line length="140rpx" :color="lineColor" :hairline="false" :dashed="dashed" v-if="line"></su-line>
    <!-- 加载中和没有更多的状态才显示两边的横线 -->
    <view :class="status == 'loadmore' || status == 'nomore' ? 'su-more' : ''" class="su-loadmore__content">
      <view class="su-loadmore__content__icon-wrap" v-if="status === 'loading' && icon">
        <su-loading-icon :color="iconColor" :size="iconSize" :mode="loadingIcon"></su-loading-icon>
      </view>
      <!-- 如果没有更多的状态下，显示内容为dot（粗点），加载特定样式 -->
      <text
        class="su-line-1"
        :style="[loadTextStyle]"
        :class="[status == 'nomore' && isDot == true ? 'su-loadmore__content__dot-text' : 'su-loadmore__content__text']"
        @tap="loadMore"
      >
        {{ showText }}
      </text>
 ...
 * @tutorial https://suni.pages.dev/sun-uni/component/loadMore.html
 ...
 * @example <su-loadmore :status="status" icon-type="iconType" load-text="loadText" />
 */
export default {
  name: 'su-loadmore',
