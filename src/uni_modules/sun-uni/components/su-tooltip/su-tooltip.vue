<template>
  <view class="su-tooltip" :style="[addStyle(customStyle)]">
    <su-overlay
      :show="showTooltip && tooltipTop !== -10000 && overlay"
      customStyle="backgroundColor: rgba(0, 0, 0, 0)"
      @click="overlayClickHandler"
    ></su-overlay>
    <view class="su-tooltip__wrapper">
      <text
        class="su-tooltip__wrapper__text"
        :id="textId"
        :ref="textId"
        :userSelect="false"
        :selectable="false"
      ...
      >
        <view class="su-tooltip__wrapper__popup" :id="tooltipId" :ref="tooltipId">
          <view
            class="su-tooltip__wrapper__popup__indicator"
            hover-class="su-tooltip__wrapper__popup__indicator--hover"
          ...
          <view class="su-tooltip__wrapper__popup__list">
            <view
              v-if="showCopy"
              class="su-tooltip__wrapper__popup__list__btn"
              hover-class="su-tooltip__wrapper__popup__list__btn--hover"
              @tap="setClipboardData"
            >
              <text class="su-tooltip__wrapper__popup__list__btn__text">复制</text>
            </view>
            <su-line direction="column" color="#8d8e90" v-if="showCopy && buttons.length > 0" length="18"></su-line>
            <block v-for="(item, index) in buttons" :key="index">
              <view class="su-tooltip__wrapper__popup__list__btn" hover-class="su-tooltip__wrapper__popup__list__btn--hover">
                <text class="su-tooltip__wrapper__popup__list__btn__text" @tap="btnClickHandler(index)">{{ item }}</text>
              </view>
              <su-line direction="column" color="#8d8e90" v-if="index < buttons.length - 1" length="18"></su-line>
            </block>
          </view>
        </view>
      </su-transition>
 ...
 * Tooltip
 * @description
 * @tutorial https://suni.pages.dev/sun-uni/component/tooltip.html
...
export default {
  ...
  methods: {
    ...
    queryRect(ref) {
      // #ifndef APP-NVUE
      // $uGetRect为uView自带的节点查询简化方法，详见文档介绍：https://suni.pages.dev/sun-uni/js/getRect.html
