<template>
  ...
  >
    <view class="su-swiper__loading" v-if="loading">
      <su-loading-icon mode="circle"></su-loading-icon>
    </view>
    <swiper
      v-else
      class="su-swiper__wrapper"
      :style="{
        flex: '1',
        height: addUnit(height)
      }"
    ...
    >
      <swiper-item class="su-swiper__wrapper__item" v-for="(item, index) in list" :key="index">
        <slot :item="item" :index="index">
          <view class="su-swiper__wrapper__item__wrapper" :style="[itemStyle(index)]">
            <!-- 在nvue中，image图片的宽度默认为屏幕宽度，需要通过flex:1撑开，另外必须设置高度才能显示图片 -->
            <image
              class="su-swiper__wrapper__item__wrapper__image"
            ...
            ></image>
            <video
              class="su-swiper__wrapper__item__wrapper__video"
              ...
              @tap="clickHandler(index)"
            ></video>
            <view v-if="showTitle && testObject(item) && item.title && testImage(getSource(item))" class="su-swiper__wrapper__item__wrapper__title">
    ...
    <view class="su-swiper__indicator" :style="[addStyle(indicatorStyle)]">
      <slot name="indicator">
        <su-swiper-indicator
 ...
 * @tutorial https://suni.pages.dev/sun-uni/component/swiper.html
 ...
 * @example	<su-swiper :list="list4" keyName="url" :autoplay="false"></su-swiper>
 */
export default {
  name: 'su-swiper',
