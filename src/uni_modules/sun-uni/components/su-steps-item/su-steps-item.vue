<template>
  <view class="su-steps-item" ref="su-steps-item" :class="[`su-steps-item--${parentData.direction}`]">
    <view
      class="su-steps-item__line"
      v-if="index + 1 < childLength"
      :class="[`su-steps-item__line--${parentData.direction}`]"
      :style="[lineStyle]"
    ></view>
    <view
      class="su-steps-item__wrapper"
      :class="[`su-steps-item__wrapper--${parentData.direction}`, parentData.dot && `su-steps-item__wrapper--${parentData.direction}--dot`]"
      :style="[itemStyleInner]"
    >
      <slot name="icon">
        <view
          class="su-steps-item__wrapper__dot"
          v-if="parentData.dot"
          :style="{
            backgroundColor: statusColor
          }"
        ></view>
        <view class="su-steps-item__wrapper__icon" v-else-if="parentData.activeIcon || parentData.inactiveIcon">
          <su-icon
        ...
        <view
          ...
          class="su-steps-item__wrapper__circle"
        >
          <text
            v-if="statusClass === 'process' || statusClass === 'wait'"
            class="su-steps-item__wrapper__circle__text"
            :style="{
    ...
    <view class="su-steps-item__content" :class="[`su-steps-item__content--${parentData.direction}`]" :style="[contentStyle]">
      <su-text
        :text="title"
      ...
      ></su-text>
      <slot name="desc">
        <su-text :text="desc" type="tips" size="12"></su-text>
      </slot>
    </view>
    <!-- <view
		    class="su-steps-item__line"
 ...
 * @tutorial https://suni.pages.dev/component/steps.html
 * @property {String}			title			标题文字
 * @property {String}			current			描述文本
 * @property {String | Number}	iconSize		图标大小  (默认 17 )
 * @property {Boolean}			error			当前步骤是否处于失败状态  (默认 false )
 * @example <su-steps current="0"><su-steps-item title="已出库" desc="10:35" ></su-steps-item></su-steps>
 */
export default {
  name: 'su-steps-item',
  ...
  methods: {
    init() {
      ...
      if (!this.parent) {
        return error('su-steps-item必须要搭配u-steps组件使用')
      }
    ...
    getStepsItemRect() {
      // #ifndef APP-NVUE
      this.$uGetRect('.su-steps-item').then((size) => {
        this.size = size
      })
      // #endif

      // #ifdef APP-NVUE
      dom.getComponentRect(this.$refs['su-steps-item'], (res) => {
        const { size } = res
        this.size = size
      })
      // #endif
