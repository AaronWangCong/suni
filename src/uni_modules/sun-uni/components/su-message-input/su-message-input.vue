<template>
  <view class="su-char-box">
    <view class="su-char-flex">
      <input :disabled="disabledKeyboard" :value="valueModel" type="number" :focus="focus" :maxlength="maxlength" class="su-input" @input="getVal" />
      <view v-for="(item, index) in loopCharArr" :key="index">
        <view
          :class="[
            breathe && charArrLength == index ? 'su-breathe' : '',
            'su-char-item',
            charArrLength === index && mode == 'box' ? 'su-box-active' : '',
            mode === 'box' ? 'su-box' : ''
          ]"
          :style="{
        ...
        >
          ...
          <view
            ...
            :class="[breathe && charArrLength == index ? 'su-breathe' : '', charArrLength === index ? 'su-middle-line-active' : '']"
            class="su-middle-line"
            :style="{ height: bold ? '4px' : '2px', background: charArrLength === index ? activeColor : inactiveColor }"
          ></view>
          <view
            v-if="mode === 'bottomLine'"
            :class="[breathe && charArrLength == index ? 'su-breathe' : '', charArrLength === index ? 'su-bottom-line-active' : '']"
            class="su-bottom-line"
            :style="{ height: bold ? '4px' : '2px', background: charArrLength === index ? activeColor : inactiveColor }"
          ></view>
          <block v-if="!dotFill">{{ charArr[index] ? charArr[index] : '' }}</block>
          <block v-else>
            <text class="su-dot">{{ charArr[index] ? '●' : '' }}</text>
          </block>
        </view>
      </view>
    </view>
 ...
 * @tutorial https://www.uviewui.com/component/messageInput.html
 ...
 * @example <su-message-input mode="bottomLine"></su-message-input>
 */
export default {
  name: 'su-message-input',
  props: {
    // 最大输入长度
    maxlength: {
      type: [Number, String],
  ...
  computed: {
    ...
    animationClass() {
      return (index) => {
        if (this.breathe && this.charArr.length == index) return 'su-breathe'
        else return ''
      }
    },
    // 用于显示字符
...
.su-box-active {
  ...
  border: 2rpx solid $su-primary;
}

.su-middle-line-active {
