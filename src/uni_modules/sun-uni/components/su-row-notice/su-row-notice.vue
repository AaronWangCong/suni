<template>
  <view class="su-notice" @tap="clickHandler">
    ...
    <view class="su-notice__content" ref="su-notice__content">
      <view ref="su-notice__content__text" class="su-notice__content__text" :style="[animationStyle]">
    ...
    <view class="su-notice__right-icon" v-if="['link', 'closable'].includes(mode)">
      ...
      <su-icon v-if="mode === 'closable'" @click="close" name="close" :size="16" :color="color"></su-icon>
    </view>
  </view>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    async vue() {
      ...
      await sleep()
      // 查询盒子和文字的宽度
      textWidth = (await this.$uGetRect('.su-notice__content__text')).width
      boxWidth = (await this.$uGetRect('.su-notice__content')).width
      // 根据t=s/v(时间=路程/速度)，这里为何不需要加上#su-notice-box的宽度，因为中设置了.su-notice-content样式中设置了padding-left: 100%
      // 恰巧计算出来的结果中已经包含了#su-notice-box的宽度
    ...
    async nvue() {
      ...
      await sleep()
      // 查询盒子和文字的宽度
      textWidth = (await this.getNvueRect('su-notice__content__text')).width
      boxWidth = (await this.getNvueRect('su-notice__content')).width
      // 将文字移动到盒子的右边沿，之所以需要这么做，是因为nvue不支持100%单位，否则可以通过css设置
      animation.transition(
        this.$refs['su-notice__content__text'],
        {
          styles: {
...
.su-notice {
  ...
  &__content {
    ...
    &__text {
      ...
      word-break: keep-all;
      white-space: nowrap;
      animation: su-loop-animation 10s linear infinite both;
      /* #endif */
      @include flex(row);
      line-height: 100%;
    }
