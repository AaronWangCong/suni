<template>
  <view class="su-tabbar-item" :style="[addStyle(customStyle)]" @tap="clickHandler">
    <view class="su-tabbar-item__icon">
      <su-icon v-if="icon" :name="icon" :color="isActive ? parentData.activeColor : parentData.inactiveColor" :size="20"></su-icon>
      <template v-else>
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      if (!this.parent) {
        error('su-tabbar-item必须搭配u-tabbar组件使用')
      }
