<template>
  <view class="su-slider" :style="[addStyle(customStyle)]">
    <template v-if="!useNative || isRange">
      <view
        ref="su-slider-inner"
        class="su-slider-inner"
        @click="onClick"
        ...
        :class="[disabled ? 'su-slider--disabled' : '']"
        :style="{
      ...
      >
        ...
        <view
          v-if="isRange"
          class="su-slider__gap su-slider__gap-0"
          :style="[
            barStyle0,
            {
              height: height,
        ...
        ></view>
        <text v-if="isRange && showValue" class="su-slider__show-range-value" :style="{ left: getPx(barStyle0.width) + getPx(blockSize) / 2 + 'px' }">
          {{ this.rangeValue[0] }}
        </text>
        <text v-if="isRange && showValue" class="su-slider__show-range-value" :style="{ left: getPx(barStyle.width) + getPx(blockSize) / 2 + 'px' }">
          {{ this.rangeValue[1] }}
        </text>
        <template v-if="isRange">
          <view
            class="su-slider__button-wrap su-slider__button-wrap-0"
            @touchstart="onTouchStart($event, 0)"
            @touchmove="onTouchMove($event, 0)"
            @touchend="onTouchEnd($event, 0)"
            @touchcancel="onTouchEnd($event, 0)"
            :style="{ left: getPx(barStyle0.width) + getPx(blockSize) / 2 + 'px' }"
          >
            <slot v-if="$slots.default || $slots.$default" />
            <view
              v-else
              class="su-slider__button"
              :style="[
                blockStyle,
                {
        ...
        <view
          class="su-slider__button-wrap"
          @touchstart="onTouchStart"
          @touchmove="onTouchMove"
          @touchend="onTouchEnd"
          @touchcancel="onTouchEnd"
          :style="{ left: getPx(barStyle.width) + getPx(blockSize) / 2 + 'px' }"
        >
          <slot v-if="$slots.default || $slots.$default" />
          <view
            v-else
      ...
      <view class="su-slider__show-value" v-if="showValue && !isRange">{{ modelValue }}</view>
    </template>
    <slider
      class="su-slider__native"
      v-else
      :min="min"
      :max="max"
      :step="step"
...
/**
 * slider 滑块选择器
 * @tutorial https://suni.pages.dev/component/slider.html
 ...
 * @example <su-slider v-model="value" />
 */
export default {
  name: 'su-slider',
  ...
  async mounted() {
    ...
    if (!this.useNative) {
      // #ifndef APP-NVUE
      this.$uGetRect('.su-slider__base').then((rect) => {
      ...
      const ref = this.$refs['su-slider__base']
      ref &&
    ...
    format(value, index = 1) {
      ...
      if (this.isRange) {
        switch (index) {
          case 0:
            return (
              Math.round(Math.max(this.min, Math.min(value, this.rangeValue[1] - parseInt(this.step), this.max)) / parseInt(this.step)) *
              parseInt(this.step)
            )
            break
          case 1:
            return (
              Math.round(Math.max(this.min, this.rangeValue[0] + parseInt(this.step), Math.min(value, this.max)) / parseInt(this.step)) *
              parseInt(this.step)
            )
            break
          default:
        ...
        return Math.round(Math.max(this.min, Math.min(value, this.max)) / parseInt(this.step)) * parseInt(this.step)
      }
    }
  }
}
