<template>
  <view class="su-rate" :id="elId" ref="su-rate" :style="[addStyle(customStyle)]">
    <view class="su-rate__content" @touchmove.stop="touchMove" @touchend.stop="touchEnd">
      <view class="su-rate__content__item cursor-pointer" v-for="(item, index) in Number(count)" :key="index" :class="[elClass]">
        <view class="su-rate__content__item__icon-wrap" ref="su-rate__content__item__icon-wrap" @tap.stop="clickHandler($event, index + 1)">
          <su-icon
        ...
        <view
          ...
          class="su-rate__content__item__icon-wrap su-rate__content__item__icon-wrap--half"
          :style="[
            {
              width: addUnit(rateWidth / 2)
            }
          ]"
          ref="su-rate__content__item__icon-wrap"
        >
          <su-icon
 ...
 * @tutorial https://suni.pages.dev/sun-uni/component/rate.html
 ...
 * @example <su-rate :count="count" :value="2"></su-rate>
 */
export default {
  name: 'su-rate',
  ...
  methods: {
    ...
    async getRateItemRect() {
      ...
      // #endif
      // #ifdef APP-NVUE
      dom.getComponentRect(this.$refs['su-rate'], (res) => {
    ...
    getRateIconWrapRect() {
      ...
      // #endif
      // #ifdef APP-NVUE
      dom.getComponentRect(this.$refs['su-rate__content__item__icon-wrap'][0], (res) => {
...
$su-rate-margin: 0 !default;
$su-rate-padding: 0 !default;
$su-rate-item-icon-wrap-half-top: 0 !default;
$su-rate-item-icon-wrap-half-left: 0 !default;

.su-rate {
  @include flex;
  align-items: center;
  margin: $su-rate-margin;
  padding: $su-rate-padding;
  /* #ifndef APP-NVUE */
  touch-action: none;
  /* #endif */
    ...
    &__item {
      ...
      &__icon-wrap {
        &--half {
          position: absolute;
          overflow: hidden;
          top: $su-rate-item-icon-wrap-half-top;
          left: $su-rate-item-icon-wrap-half-left;
        }
      }
    }
  }
