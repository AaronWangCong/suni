    ...
    <page-wraper>
      <view class="icon">
        <view style="position: sticky; top: 0; z-index: 2">
          <su-search hide-cancel placeholder="查找图标" light v-model="keyword" @search="handleSearch" @clear="handleClear" />
        </view>
        <view class="icon-list">
          <view v-for="(icon, index) in showIcons" :key="index" class="icon-item" @click="handleClick(icon)">
            <view><su-icon :name="icon" size="22px" custom-class="icon-item-class" /></view>
            <view class="icon-item-name">{{ icon }}</view>
          </view>
          <su-status-tip v-if="!showIcons.length" image="search" tip="当前无相关图标" />
        </view>
      </view>
    </page-wraper>
  </view>
...
function handleClick(icon: string) {
  ...
  uni.setClipboardData({
    data: `<su-icon name="${icon}" size="22px"></su-icon>`,
    showToast: false,
    success: () => {
      showNotify({
        type: 'success',
        duration: 1500,
        message: `复制成功: <su-icon name="${icon}" size="22px"></su-icon>`
      })
    }
  })
