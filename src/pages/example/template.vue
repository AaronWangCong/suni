<template>
  <view class="wrap">
    ...
    <view class="list-wrap">
      <up-cell-group title-bg-color="rgb(243, 244, 246)" :title="getGroupTitle(item)" v-for="(item, index) in list" :key="index">
        <up-cell
        ...
        >
          <template v-slot:icon>
            <image class="su-cell-icon" :src="getIcon(item1.icon)" mode="widthFix"></image>
          </template>
        </up-cell>
      </up-cell-group>
    </view>
...
export default {
  ...
  methods: {
    getIcon(path) {
      return 'https://suni.pages.dev/h5/static/uview/demo/' + path + '.png'
    },
    openPage(path) {
      this.$u.route({
