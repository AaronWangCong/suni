<template>
  <page-wraper show-dark-mode>
    <su-message-box />
    <su-toast />
    <su-action-sheet v-model="showAction" :actions="actions" />

    <form @submit="formSubmit">
      <su-cell-group custom-class="group" title="基础信息" border>
        <su-input
          label="优惠券名称"
          label-width="100px"
          :maxlength="20"
        ...
        />
        <su-picker label="优惠方式" label-width="100px" name="promotion" align-right v-model="promotion" :columns="promotionlist" />
        <su-cell title="券面额" required title-width="100px" custom-value-class="cell-left">
      ...
      <su-cell-group custom-class="group" title="时间和地址" border>
        <su-datetime-picker label="时间" label-width="100px" name="date" v-model="date" @confirm="handleDate" />
        <su-col-picker
          label="地址"
          label-width="100px"
          name="address"
      ...
      <su-cell-group custom-class="group" title="其他信息" border>
        <su-input
          label="活动细则"
          label-width="100px"
          type="textarea"
        ...
        />
        <su-cell title="发货数量" center>
          <su-input-number v-model="count" name="count" @change="handleCount" />
        </su-cell>
        <su-cell title="这里显示的是多文字标题包含非常的文字" title-width="240px" center>
          <su-switch v-model="switchVal" name="switchVal" @change="handleSwitch" />
        </su-cell>
        <su-input
          label="歪比巴卜"
          label-width="100px"
          ...
          @change="handleCardId"
        />
        <su-input label="玛卡巴卡" label-width="100px" name="phone" placeholder="请输入玛卡巴卡" clearable v-model="phone" @change="handlePhone" />
      </su-cell-group>
      <view class="tip">
        <su-checkbox v-model="read" name="read" @change="handleRead" custom-label-class="label-class">
          已阅读并同意
          <text style="color: #4d80f0">《巴拉巴拉吧啦协议》</text>
        </su-checkbox>
      </view>
      <view class="footer">
        <su-button block round size="large">提交</su-button>
      </view>
    </form>
  </page-wraper>
</template>
<script setup lang="ts">
import { useToast, useMessage } from '@/uni_modules/suni'
import type { ColPickerColumnChangeOption } from '@/uni_modules/suni/components/su-col-picker/types'
import { ref } from 'vue'
