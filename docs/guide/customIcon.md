...
sun-uni已通过大量的实践中，收集了用户最有可能需要用到的图标，见[Icon 图标](/component/icon.html)，但我们也相信，它肯定无法覆盖所有的场景和需求。
...
```html
<su-icon name="copy" custom-prefix="custom-icon"></su-icon>
```
...
8. 在页面通过uView的[Icon](/component/icon.html)组件使用图标，图标名称为您在阿里图标库中点击"编辑图标"时的"Font Class / Symbol"(该值可修改，每次修改都需重新下载"iconfont.css"放到uni-app目中，
覆盖原来的"iconfont.css")
...
```html
<su-icon name="backspace" custom-prefix="custom-icon" size="30" color="#888888"></su-icon>
```
