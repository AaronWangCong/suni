...
Suni 默认使用中文语言，同时支持多语言切换，如果你希望使用其他语言，你可以参考下面的方案。
...
export default defineConfig({
  ...
  optimizeDeps: {
    exclude: process.env.UNI_PLATFORM === 'h5' && process.env.NODE_ENV === 'development' ? ['suni'] : []
  }
  ...
})
...
使用[optimizeDeps.exclude](https://cn.vitejs.dev/config/dep-optimization-options.html#optimizedeps-exclude)在预构建中强制排除`suni`模块，在`uni_modules`模式下，不需要做任何处理。
...
```typescript
import { Locale } from 'suni'
// 引入英文语言包
import enUS from 'suni/locale/lang/en-US'
