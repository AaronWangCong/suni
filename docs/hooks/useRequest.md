## [useRequest](https://github.com/AaronWangCong/suni/blob/main/src/uni_modules/sun-uni/hooks/request/useRequest.ts)
...
- `run` 执行请求的函数，接受可变参数
- `runAsync` 异步执行请求的函数，接受可变参数
- `data` 接口返回的数据
- `error` 接口返回的错误信息
- `loading` 是否正在请求中
- `params` 当次执行的 service 的参数数组
- `mutate` 更新数据的方法，接受一个函数或新的数据对象
- `refresh` 重新请求
- `refreshAsync` 异步执行重新请求

### 类型命名空间 [SuRequest](https://github.com/AaronWangCong/suni/blob/main/src/uni_modules/sun-uni/hooks/request/index.d.ts)
