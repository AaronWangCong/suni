...
在uView中，一些组件如`su-popup`、`su-keyboard`组件等，提供了一个`safe-area-inset-bottom`参数(布尔类型)，如果设置为`true`，就会在组件内部元素中添加一个
`safe-area-inset-bottom`样式类名，从而避免安全区指示条引起的问题，以下为uView的`su-keyboard`组件在`微信浏览器`中分别设置`safe-area-inset-bottom`参数
为`false`和`true`的表现：
