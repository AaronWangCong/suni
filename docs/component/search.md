...
<demo-model url="/pages/components/search/index"></demo-model>
...
```html
<template>
  <su-search placeholder="日照香炉生紫烟" v-model="keyword"></su-search>
</template>
...
```html
<su-search shape="round"></su-search>
```
...
```html
<su-search :clearabled="true"></su-search>
```
...
```html
<su-search :show-action="true" action-text="搜索" :animation="true"></su-search>
```
...
```html
<su-search input-align="center" height="70"></su-search>
```

### 示例源码

[点击可以查看](https://github.com/AaronWangCong/suni/blob/main/src/pages/components/search/search.nvue) 右侧演示页面的源码
