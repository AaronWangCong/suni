...
<demo-model url="/pages/components/indexList/indexList"></demo-model>
...
外层包裹一个`index-list`组件，内部锚点通过`index-anchor`组件传入，自定义列表内容通过`index-item`嵌套使用
...
```html
<template>
  <su-index-list :index-list="indexList">
    <template v-for="(item, index) in itemArr">
      <!-- #ifdef APP-NVUE -->
      <su-index-anchor :text="indexList[index]"></su-index-anchor>
      <!-- #endif -->
      <su-index-item>
        <!-- #ifndef APP-NVUE -->
        <su-index-anchor :text="indexList[index]"></su-index-anchor>
        <!-- #endif -->
        <view class="list-cell" v-for="(cell, index) in item">{{cell}}</view>
      </su-index-item>
    </template>
  </su-index-list>
</template>
...
[点击可以查看](https://github.com/AaronWangCong/suni/blob/main/src/pages/components/indexList/indexList.nvue) 右侧演示页面的源码
...
| customNavHeight| 自定义导航栏的高度，单位默认px | Number \| String | 0 | - |
...
| size | 表锚点文字大小，单位默认px | String \| Number | 14 | - |
