...
<demo-model url="/pages/components/fullScreen/index"></demo-model>
...
"pages": [
	{
		"path": "uview-ui/component/su-full-screen/su-full-screen",
		"style": {
...
export default {
	onLoad() {
		...
		this.$u.route("/uview-ui/component/su-full-screen/su-full-screen?id=1");
	}
}
```
...
Tabbar栏，所以这里我们可以使用uView的[Popup 弹出层](/component/popup.html)组件，并且将`popup`组件的`mode`参数设置`center`，即中部弹出的形式。

下方示例为打开一个[Modal 模态框](/component/modal.html)组件的示例，此组件内部用的也是`popup`组件。

```html
<template>
	<su-modal v-model="show" :show-cancel-button="true" confirm-text="升级"
		title="发现新版本" @cancel="cancel" @confirm="confirm"
	>
		<view class="su-update-content">
