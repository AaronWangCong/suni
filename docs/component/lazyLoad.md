...
<demo-model url="/pages/components/lazyload/index"></demo-model>
...
<template>
	<view>
		<su-lazy-load v-for="(item, index) in list" :key="index" :image="item.src"></su-lazy-load>
	</view>
</template>
<script>
	export default {
...
<template>
	<view>
		<su-lazy-load :image="image" :loading-img="loadingImg" :error-img="errorImg"></su-lazy-load>
	</view>
</template>
<script>
	export default {
...
```html
<su-lazy-load :image="image" threshold="300"></su-lazy-load>
```

### 示例源码

[点击可以查看](https://github.com/AaronWangCong/suni/blob/main/src/pages/components/lazyLoad/lazyLoad.nvue) 右侧演示页面的源码
