...
<demo-model url="/pages/components/waterfall/index"></demo-model>

这是一个瀑布流形式的组件，内容分为左右两列，结合sun-uni的`懒加载`组件效果更佳。
相较于某些只是奇偶数左右分别，或者没有利用vue作用域插槽的做法，sun-uni的瀑布流实现了真正的 组件化，搭配`LazyLoad 懒加载`和`loadMore 加载`更多组件，让您开箱即用，眼前一亮。
...
<template>
  <view class="wrap">
    <su-button @click="clear">清空列表</su-button>
    <su-waterfall v-model="flowList" ref="uWaterfall1">
      <template v-slot:left="{leftList}">
        <view class="demo-warter" v-for="(item, index) in leftList" :key="index">
          <!-- 警告：微信小程序中需要hx2.8.11版本才支持在template中结合其他组件，比如下方的lazy-load组件 -->
          <su-lazy-load threshold="-450" border-radius="10" :image="item.image" :index="index"></su-lazy-load>
          ...
          <su-icon name="close-circle-fill" color="#fa3534" size="34" class="su-close" @click="remove(item.id)"></su-icon>
        </view>
      </template>
      <template v-slot:right="{rightList}">
    ...
    <su-loadmore bg-color="rgb(240, 240, 240)" :status="loadStatus" @loadmore="addRandomData"></su-loadmore>
  </view>
</template>
...
<script>
  export default {
    ...
    methods: {
      ...
      remove(id) {
        this.$refs.uWaterfall1.remove(id)
      },
      clear() {
...
<style lang="scss" scoped>
  ...
  .demo-tag-owner {
    background-color: $su-type-error;
    color: #ffffff;
    display: flex;
    align-items: center;
    padding: 4rpx 14rpx;
  ...
  .demo-tag-text {
    border: 1px solid $su-type-primary;
...
1. 上方的示例中，结合了uView的[lazyload懒加载](/component/lazyLoad.html)和[loadmore加载更多](/component/loadMore.html)组件，具体用法，请见文档。
...
[点击可以查看](https://github.com/AaronWangCong/suni/blob/main/src/pages/components/waterfall/waterfall.nvue) 右侧演示页面的源码
